...
function LanguageSection() {
  ...
  function handleChange(value: string) {
    mutation.mutate({ locale: value as "de-DE" | "en-US" })
  }
  ...
  return (
    <div className="rounded-xl border bg-card" style={{ borderColor: "hsl(var(--border))" }}>
      ...
      <div style={{ padding: "16px 24px 20px" }}>
        ...
        <Select value={currentLocale} onValueChange={handleChange} disabled={mutation.isPending}>
          <SelectTrigger className="h-9 w-full" style={{ maxWidth: 240, opacity: mutation.isPending ? 0.6 : 1 }}>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="de-DE">{t("profile.languageOptions.de-DE")}</SelectItem>
            <SelectItem value="en-US">{t("profile.languageOptions.en-US")}</SelectItem>
          </SelectContent>
        </Select>
      </div>
    </div>
