...
function PlayersPage() {
  ...
  function openSheet() {
    navigate({ search: (prev) => ({ ...prev, edit: "new" }) })
  }
  ...
  const isDirty = form.firstName !== "" || form.lastName !== ""
  ...
  function navigateToPlayer(playerId: string) {
    navigate({ to: "/$orgSlug/players/$playerId", params: { orgSlug, playerId } })
  }
      ...
      >
        ...
        {teamFilter.length > 0 && (
          <div className="w-20 shrink-0 hidden md:block">
            {player.currentTeam ? (
              <Link
                to="/$orgSlug/teams/$teamId"
                params={{ orgSlug, teamId: player.currentTeam.id }}
                onClick={(e) => e.stopPropagation()}
              >
                <Badge variant="outline" className="text-xs font-normal cursor-pointer hover:bg-accent/10">
    ...
    <>
      ...
      >
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            action={
              <Button variant="accent" onClick={openSheet} disabled={atPlayerLimit}>
                <Plus className="mr-2 h-4 w-4" aria-hidden="true" />
      ...
      <Sheet open={sheetOpen} onOpenChange={(open) => { if (!open) closeSheet() }} dirty={isDirty} onDirtyClose={() => setConfirmCloseOpen(true)}>
        <SheetContent>
          <SheetClose />
          <SheetHeader>
            <SheetTitle>{t("playersPage.dialogs.newTitle")}</SheetTitle>
            <SheetDescription>{t("playersPage.dialogs.newDescription")}</SheetDescription>
          </SheetHeader>
