...
function GameDetailPage() {
    ...
    return (
      <SectionWrapper>
        <div className="text-center py-12">
          ...
          <Link to="/schedule" className="text-web-primary hover:underline">
  ...
  return (
    <div className="animate-fade-in">
      <SectionWrapper>
        <Link to="/schedule" className="inline-flex items-center gap-1 text-sm text-web-text/50 hover:text-web-primary mb-6">
          <ArrowLeft className="h-4 w-4" />
          Zurück zum Spielplan
        </Link>

        {/* Round info */}
        <div className="text-sm text-web-text/50 mb-2 text-center">
          {game.round.division?.name && <span>{game.round.division.name} &middot; </span>}
          {game.round.name}
        </div>

        {/* Score header */}
        <div className="bg-white rounded-xl border border-web-text/10 p-6 sm:p-8 mb-8">
          <div className="flex items-center justify-center gap-6 sm:gap-10">
            ...
            <div className="text-center flex-1">
              ...
              <div className="text-xs text-web-text/50">Heim</div>
            </div>
            ...
            <div className="text-center flex-1">
              ...
              <div className="text-xs text-web-text/50">Gast</div>
            </div>
          </div>

          {game.scheduledAt && (
            <div className="text-center text-sm text-web-text/50 mt-4">
        ...
        {goals.length > 0 && (
          <div className="mb-8">
            ...
            <div className="rounded-lg border border-web-text/10 bg-white overflow-hidden divide-y divide-web-text/5">
              {goals.map((event) => {
                  ...
                  <div key={event.id} className={cn("flex items-center px-4 py-3 text-sm", isHome ? "flex-row" : "flex-row-reverse")}>
                    <div className={cn("flex-1", isHome ? "text-left" : "text-right")}>
                      ...
                      {(event.assist1 || event.assist2) && (
                        <span className="text-web-text/50 ml-2 text-xs">
                          (
                          {[event.assist1, event.assist2]
                            .filter(Boolean)
                    ...
                    <div className="px-4 text-web-text/50 text-xs tabular-nums font-medium">
              ...
              {penalties.map((event) => (
                <div key={event.id} className="flex items-center px-4 py-3 text-sm">
                  <div className="flex-1">
                    ...
                    <span className="text-web-text/50 ml-2 text-xs">({event.team.shortName})</span>
                  </div>
                  <div className="text-web-text/60 text-xs">
                    {event.penaltyMinutes && <span>{event.penaltyMinutes} Min</span>}
                    {event.penaltyType && <span> &middot; {event.penaltyType.name}</span>}
                  </div>
                  <div className="pl-4 text-web-text/50 text-xs tabular-nums">
                    {event.period}. D &middot; {String(event.timeMinutes).padStart(2, "0")}:
                    {String(event.timeSeconds).padStart(2, "0")}
                  </div>
                </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <div key={team.id} className="rounded-lg border border-web-text/10 bg-white overflow-hidden">
                  <div className="px-4 py-2 bg-web-text/[0.03] font-semibold text-sm flex items-center gap-2">
                    <TeamLogo name={team.name} logoUrl={team.logoUrl} size="sm" />
                    {team.shortName}
                  </div>
                  <div className="divide-y divide-web-text/5">
                    {lineup.map((l) => (
                      <div key={l.id} className="flex items-center px-4 py-2 text-sm">
                        <span className="w-8 text-web-text/40 tabular-nums font-medium">
                          {l.jerseyNumber ?? "-"}
                        </span>
                        <span className="flex-1">
                          {l.player.firstName} {l.player.lastName}
                        </span>
                        <span className="text-xs text-web-text/40 uppercase">{l.position}</span>
                      </div>
                    ))}
                    {lineup.length === 0 && (
                      <div className="px-4 py-4 text-sm text-web-text/40 text-center">Keine Aufstellung verfügbar</div>
                    )}
                  </div>
                </div>
              ))}
