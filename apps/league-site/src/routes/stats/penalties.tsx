...
const PenaltyTypeChart = lazy(() =>
  import("~/components/charts/penaltyTypeChart").then((m) => ({ default: m.PenaltyTypeChart })),
)
    ...
    <StatsPageShell title={t.statsPenalties.title} selectedSeasonId={selectedSeasonId} onSeasonChange={setSeasonId}>
      ...
      {features.advancedStats && !isLoading && teamPenaltyStats && teamPenaltyStats.length > 0 && (
        <div className="mb-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <ChartSuspense>
            <PenaltyTeamChart stats={teamPenaltyStats} title={t.statsPenalties.byTeam} />
          </ChartSuspense>
          <ChartSuspense>
            <PenaltyTypeChart stats={teamPenaltyStats} title={t.statsPenalties.types} />
          </ChartSuspense>
        </div>
      )}
      {isLoading ? (
