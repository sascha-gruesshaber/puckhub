...
export const assistsSearchValidator = (
  ...
  return (
    <StatsPageShell title={t.statsAssists.title} selectedSeasonId={selectedSeasonId} onSeasonChange={setSeasonId}>
      ...
      {features.advancedStats && !isLoading && sorted.length > 0 && (
        <div className="mb-8">
          <ChartSuspense>
            <ScorerChart stats={sorted} mode="assists" title={t.statsAssists.topAssists} limit={10} />
          </ChartSuspense>
        </div>
      )}
      {isLoading ? <StatsTableSkeleton /> : <PlayerTable stats={sorted} sortBy="assists" />}
    </StatsPageShell>
  )
}
