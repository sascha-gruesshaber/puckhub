...
export const assistsSearchValidator = (s: Record<string, unknown>): { season?: string; team?: string; position?: string } => ({
...
export const Route = createFileRoute("/stats/assists")({
  component: AssistsPage,
  head: () => ({ meta: [{ title: "Vorlagen-Statistiken" }] }),
  validateSearch: assistsSearchValidator,
})

export function AssistsPage() {
  const t = useT()
  const lp = useLocalePath()
  const visible = useSubRouteVisible("/stats/assists")
  const org = useOrg()
  ...
  const sorted = useMemo(() => {
    ...
    return [...playerStats].sort((a, b) => b.assists - a.assists || b.totalPoints - a.totalPoints)
  }, [playerStats])

  if (visible === false) return <Navigate to={lp("/stats")} replace />

  return (
    <StatsPageShell title={t.statsAssists.title} selectedSeasonId={selectedSeasonId} onSeasonChange={setSeasonId}>
      <StatsFilterBar teamOptions={teamOptions} teamValue={selectedTeamId} onTeamChange={setTeamId} position={selectedPosition} onPositionChange={setPosition} showPosition />
      {features.advancedStats && !isLoading && sorted.length > 0 && (
        <div className="mb-8"><ChartSuspense><ScorerChart stats={sorted} mode="assists" title={t.statsAssists.topAssists} limit={10} /></ChartSuspense></div>
      )}
