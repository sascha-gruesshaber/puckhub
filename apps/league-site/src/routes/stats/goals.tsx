...
export const goalsSearchValidator = (s: Record<string, unknown>): { season?: string; team?: string; position?: string } => ({
  ...
  return (
    <StatsPageShell title={t.statsGoals.title} selectedSeasonId={selectedSeasonId} onSeasonChange={setSeasonId}>
      ...
      {features.advancedStats && !isLoading && sorted.length > 0 && (
        <div className="mb-8"><ChartSuspense><ScorerChart stats={sorted} mode="goals" title={t.statsGoals.topScorers} limit={10} /></ChartSuspense></div>
      )}
