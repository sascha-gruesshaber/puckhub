...
import { ArrowLeft, Globe, Mail, MapPin, Shield, User } from "lucide-react"
...
export const Route = createFileRoute("/teams/$teamId")({
  ...
  validateSearch: (s: Record<string, unknown>): { tab?: string; from?: string } => ({
...
function TeamDetailPage() {
  ...
  const { tab: tabParam, from: fromParam } = Route.useSearch()
  ...
  const TABS = [
    { value: "roster", label: t.teams.roster },
    { value: "history", label: t.teams.seasonHistory },
  ] as const

  const activeTab = tabParam === "history" ? "history" : "roster"
  const backPath = useBackPath()

  const setTab = (tab: string) =>
    navigate({ search: { tab: tab === "roster" ? undefined : tab, from: fromParam }, replace: true })
  ...
  // Only fetch history data when the history tab is active (or when advancedStats is enabled)
  const { data: historyData, isLoading: historyLoading } = trpc.publicSite.getTeamHistory.useQuery(
    { organizationId: org.id, teamId },
    { enabled: features.advancedStats && activeTab === "history", staleTime: 300_000 },
  )
    ...
    <div className="animate-fade-in">
      <SectionWrapper>
        ...
        <div className="bg-league-surface rounded-xl border border-league-text/10 p-6 sm:p-8 mb-8">
          ...
          {team.teamPhotoUrl && (
            <img
              ...
              alt={`${team.name} ${t.teams.teamPhoto}`}
        ...
        {features.advancedStats && (
          <div className="inline-flex rounded-lg bg-league-text/[0.06] p-1 mb-8">
            {TABS.map((tab) => (
              <button
                key={tab.value}
                type="button"
                ...
                className={cn(
                  ...
                  activeTab === tab.value
                    ...
                    : "text-league-text/50 hover:text-league-text hover:bg-league-text/[0.06]",
                )}
              >
                {tab.label}
              </button>
        ...
        {/* History tab */}
        {activeTab === "history" && features.advancedStats && (
            ...
            {historyLoading ? (
              ...
              <>
                ...
                {historySeasonsData.length > 0 ? (
                  <SeasonTimeline seasons={historySeasonsData} topScorers={historyTopScorers} topGoalies={historyTopGoalies} />
                ) : (
                  <EmptyState
