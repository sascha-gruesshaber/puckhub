  ...
  const sectionRenderers: Record<string, () => React.ReactNode> = {
    ...
    results: () => (
      <SectionWrapper
        ...
        action={
          <Link to="/schedule" className="text-sm text-league-primary hover:underline">
            Alle Ergebnisse
          </Link>
        }
      >
    ...
    news: () => (
      <SectionWrapper
        ...
        title="Neuigkeiten"
        action={
          <Link to="/news" className="text-sm text-league-primary hover:underline">
            Alle News
          </Link>
        }
      >
    ...
    standings: () => (
      <SectionWrapper
        ...
        className="bg-league-text/[0.02]"
        action={
          <Link to="/standings" className="text-sm text-league-primary hover:underline">
              ...
              <tbody>
                {data.standings.map((s, i) => (
                  <tr key={s.id} className="border-t border-league-text/5 hover:bg-league-text/[0.02]">
                    <td className="px-4 py-2 text-league-text/50 font-medium">{i + 1}</td>
  ...
  return (
    <section className="relative bg-league-header-bg text-league-header-text">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
        ...
        <p className="text-lg sm:text-xl text-league-header-text/70 max-w-2xl">
          Aktuelle Ergebnisse, Tabellen, Spielpläne und News
        </p>
        <div className="mt-8 flex gap-3">
          <Link
            to="/standings"
            className="inline-flex items-center gap-2 rounded-lg bg-league-primary px-5 py-2.5 text-sm font-semibold text-white transition-all hover:opacity-90"
          >
