...
function GoalieChart({ stats, title }: GoalieChartProps) {
  const option = useMemo(() => {
    ...
    const colors = values.map((v) => {
      ...
      if (ratio < 0.4) return LEAGUE_CHART_COLORS[3]
      if (ratio < 0.7) return LEAGUE_CHART_COLORS[1]
      return LEAGUE_CHART_COLORS[2]
    })
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (params: Array<{ name: string; value: number }>) => {
          ...
          return `${p?.name}<br/>GAA: <b>${p?.value?.toFixed(2)}</b><br/>Spiele: ${original?.gamesPlayed ?? 0}`
        },
      },
      grid: { top: 40, right: 40, bottom: 10, left: 20, containLabel: true },
      xAxis: { type: "value", name: "GAA", nameLocation: "end" },
